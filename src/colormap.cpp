...
void convert_image_float(const cv::Mat &input_image, cv::Mat &output_image, const float vmin, const float vmax, const std::array<std::array<uint8_t, 3>, 256> &jet)
{
    // Convert a CV_16UC1 to a CV_8C3
    output_image = cv::Mat(input_image.rows, input_image.cols, CV_8UC3);
...
#pragma omp parallel for
        for (int c = 0; c < input_image.cols; c++)
        {
            float pixel = input_image.at<float>(r, c);
            if (pixel == 0)
            {
                output_image.at<cv::Vec3b>(r, c) = {0, 0, 0};
            }
